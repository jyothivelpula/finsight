...
import { aiApi, analyticsApi, financeApi } from "../api/finsight";
...
export default function Dashboard() {
  ...
  const [chatMessages, setChatMessages] = useState<
    { role: "user" | "assistant"; text: string; suggestedActions?: string[] }[]
  >([
    {
      role: "assistant",
      text: `Hi ${firstName}! I'm your AI financial coach. Ask me anything about your money. I can help you analyze, plan and improve your finances.`,
      suggestedActions: AI_CHIPS,
    },
  ]);
  ...
  const askChat = async (q: string) => {
    ...
    setChatLoading(true);
    try {
      const res = await aiApi.ask(q, history, year, month);
      setChatMessages((m) => [
        ...m,
        {
          role: "assistant",
          text: res.answer,
          suggestedActions: res.suggested_actions?.length ? res.suggested_actions : AI_CHIPS,
        },
      ]);
    } catch {
      setChatMessages((m) => [
        ...m,
        { role: "assistant", text: "I couldn’t reach the AI service right now. Try again shortly." },
      ]);
    } finally {
    ...
    <div className="space-y-5">
      ...
      <div className="grid gap-4 xl:grid-cols-2">
        ...
        <Panel className="flex flex-col overflow-hidden p-0">
          ...
          <div className="max-h-64 flex-1 space-y-3 overflow-y-auto px-4 py-4">
            ...
            <div className="flex flex-col gap-2">
              {latestSuggestions.slice(0, 3).map((chip) => (
                <button
                  ...
                  className="flex items-center justify-between rounded-xl border border-line bg-white px-3 py-2 text-left text-xs font-medium text-ink hover:border-moss/40 hover:bg-soft"
                >
                  <span>{chip}</span>
