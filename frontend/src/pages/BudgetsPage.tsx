  ...
  return (
    <div className="mx-auto max-w-6xl">
      <div className="mb-8 flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div>
          <h1 className="text-4xl font-semibold tracking-tight text-white">Budgets</h1>
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        <div className="rounded-2xl border border-line bg-card px-5 py-5">
          ...
          <p className="mt-3 text-3xl font-semibold tracking-tight text-leaf">{money(totalBudget)}</p>
        </div>
        <div className="rounded-2xl border border-line bg-card px-5 py-5">
          <p className="text-[11px] font-semibold uppercase tracking-[0.16em] text-muted">
            Spent so far
          </p>
          <p className="mt-3 text-3xl font-semibold tracking-tight text-white">{money(totalSpent)}</p>
        </div>
        <div className="rounded-2xl border border-line bg-card px-5 py-5">
          <p className="text-[11px] font-semibold uppercase tracking-[0.16em] text-muted">
            Avg. usage
          </p>
          <p className="mt-3 text-3xl font-semibold tracking-tight text-white">{pct(avgUsage)}</p>
        </div>
      </div>
      ...
      <div className="mt-5 min-h-[320px] rounded-2xl border border-line bg-card">
        {filtered.length === 0 ? (
          <div className="flex min-h-[320px] flex-col items-center justify-center px-6 py-16 text-center">
            <p className="text-xl font-semibold text-white">No budget records</p>
                ...
                <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                  <div>
                    <div className="flex items-center gap-3">
                      <p className="font-semibold text-white">{nameOf(b.category_id)}</p>
        ...
        <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/70 p-4 sm:items-center">
          <div className="w-full max-w-md rounded-2xl border border-line bg-[#111816] p-5 shadow-2xl sm:p-6">
            ...
            <form onSubmit={onSubmit} className="space-y-4">
              ...
              <label className="block space-y-2">
                <span className="text-sm text-white/85">Budget amount (₹)</span>
                <input
                  type="number"
                  min="1"
                  required
